...
// `residual` is what the user's brightness is minus what the schedule wanted,
// with the existing offset already in the target. Adding it whole leaves the
// next target exactly where they put it, so the loop has nothing left to
// correct and stays quiet.
function learn(learned, phaseKey, residual) {
  var table = learnedTable(learned);
  var delta = Number(residual);
  if (PHASE_KEYS.indexOf(phaseKey) < 0 || !isFinite(delta)) return table;
  table[phaseKey] = clamp(table[phaseKey] + delta, -LEARN_LIMIT, LEARN_LIMIT);
  return table;
}
