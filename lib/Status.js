...
function sentence(state) {
  ...
  // Two different ways of not running, and they need different words now that
  // one of them ends by itself. "Paused" used to mean the toggle was off; it
  // means the overnight hold, and the toggle says "Off".
  if (s.pausedUntilText) {
    return "Paused until sunrise at " + s.pausedUntilText
      + ". Your brightness stays wherever you put it until then.";
  }

  if (!s.automatic) return "Off. Your brightness stays wherever you put it.";
  ...
  return lead + learnedClause(s.learnedBand, s.learnedOffset);
}
...
function learnedClause(learnedBand, learnedOffset) {
  var noun = PHASE_NOUNS[learnedBand];
